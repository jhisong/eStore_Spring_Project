...
cartApp
		.controller(
				...
				function($scope, $http) {
					...
					$scope.plusToCart = function(productId) {
						$scope.setCsrfToken();
						if ($scope.cart.cartItems[productId-1].quantity <= $scope.cart.cartItems[productId-1]
								.product.unitInStock - 1) {
							$http.put('/eStore/api/cart/plus/' + productId)
									.then(function successCallback() {
										$scope.refreshCart();
									}, function errorCallback() {
										alert("Adding to the cart failed")
					...
					$scope.minusToCart = function(productId) {
						$scope.setCsrfToken();

						if ($scope.cart.cartItems[productId - 1].quantity >= 1) {
							$http.put('/eStore/api/cart/sub/' + productId)
									.then(function successCallback() {
										$scope.refreshCart();
									}, function errorCallback() {
										alert("Adding to the cart failed")
